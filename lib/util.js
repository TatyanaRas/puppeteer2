...
module.exports = {
  ...
  cheque: async function (page, row, ...seats) {
    ...
    try {
      for (let i = 0; i < seats.length; i++) {
        await clickElement(
          page,
          `div.buying-scheme__wrapper > div.buying-scheme__row:nth-child(${row}) > span.buying-scheme__chair:nth-child(${seats[i]})`
        );
        await page.waitForSelector(
          `div.buying-scheme__wrapper > div.buying-scheme__row:nth-child(${row}) > span.buying-scheme__chair:nth-child(${seats[i]}).buying-scheme__chair_selected`
        );
      }
    } catch (error) {
  ...
  checkSeatIsTaken: async function (page, row, ...seats) {
    ...
    try {
      for (let i = 0; i < seats.length; i++) {
        await page.waitForSelector(
          `div.buying-scheme__wrapper > div.buying-scheme__row:nth-child(${row}) > span.buying-scheme__chair:nth-child(${seats[i]}).buying-scheme__chair_taken`
        );
      }
    } catch (error) {
